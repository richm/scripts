...
for (uid,dn) in userids.iteritems():
    print "Get the DS DN of", uid, " ", dn
    ents = ds.search_s(suffix, scope, "(uid=%s)" % uid);
    ent = ents[0]
    mod = [(ldap.MOD_ADD, 'userPassword', 'password'),
           (ldap.MOD_REPLACE, 'userPassword', 'pwd2'),
           (ldap.MOD_REPLACE, 'userPassword', None)]
    ds.modify_s(ent.dn, mod)
    print "added userPassword in", dn
